...
import soct.antlr.verilog.Verilog2001BaseListener;
import soct.antlr.verilog.Verilog2001Lexer;
import soct.antlr.verilog.Verilog2001Parser;
import soct.antlr.verilog.Verilog2001Parser.*;
...
public class WrapVHDL {
    ...
    private static void ln(String s) {
        if (output_file != null) {
            try {
                // Append to the file
                java.nio.file.Files.writeString(output_file.toPath(), s + '\n', java.nio.file.StandardOpenOption.APPEND);
            } catch (Exception x) {
                throw new Error(x);
            }
        } else {
    ...
    private static void getBusSignals() {
        ...
        for (PortDeclaration d : l) {
            ...
            else if (sig_name.startsWith("l2_frontend_bus_axi4_") && !sig_name.startsWith("l2_frontend_bus_axi4_0_"))
                multi_dma = true;
        }
        for (PortDeclaration d : l) {
            String sig_name = d.id.getText();
            ...
            for (int i = 0; i < atts.length; i += 2) {
                if (sig_name.endsWith("_" + atts[i])) {
                    ...
                    if (bus_name.equals("jtag")) {
                        ...
                        if (axi_bus.addr_range == null && sig.xilinx_name.endsWith("ADDR"))
                            axi_bus.addr_range = sig.range;
                        if (axi_bus.data_range == null && sig.xilinx_name.endsWith("DATA"))
                            axi_bus.data_range = sig.range;
                    }
                }
            }
        }
    ...
    private static void generateEntityPort() {
        ln("");
        ln("entity " + riscv_module_name + " is");
        if (mem_addr_offset) {
            ln("generic (");
    ...
    private static void generateRocketSystemComponent() {
        ...
        for (PortDeclaration decl : decl_list) {
            ...
            if (sig != null) dst = sig.addr_offs_name != null ? sig.addr_offs_name : sig.signal_name;
            ...
            else if (nm.equals("debug_reset")) dst = "debug_reset";
                ...
                // Fall-back: everything that ends in _clock gets connected to system clock
            else if (nm.endsWith("_clock")) {
                System.err.println("[Fall-back] Mapping " + nm + " to clock");
                dst = "clock";
            }
            // Fall-back: everything that ends in _reset gets connected to system reset
            else if (nm.endsWith("_reset")) {
                System.err.println("[Fall-back] Mapping " + nm + " to reset");
                dst = "reset";
            } else {
                dst = nm;
                System.err.println("[Fall-back] Mapping " + nm + " to " + dst);
            }
            if (s != null) ln(s + ',');
    ...
    private static void generateSystemVHDL() {
        ...
        ln("");
        ln("architecture Behavioral of " + riscv_module_name + " is");
    ...
    public static void main(String[] args) {
        ...
        while (arg_pos < args.length) {
            String mode = args[arg_pos++];

            if (mode.equals("-m")) {
                if (arg_pos < args.length) {
                    riscv_module_name = args[arg_pos++];
                } else {
                    System.err.println("Error: Missing module name after -m");
                    success = false;
                    break;
                }
            } else if (mode.equals("-o")) {
                if (arg_pos < args.length) {
                    String outputFileName = args[arg_pos++];
                    output_file = new File(outputFileName);
                    try {
                        System.out.println("Writing output to " + output_file.getAbsolutePath());
                        if (output_file.exists()) {
                            success &= output_file.delete();
                        }
                        success &= output_file.createNewFile();
                    } catch (IOException e) {
                    ...
                    System.err.println("Error: Missing output file name after -o");
                    success = false;
                    break;
                }
            } else if (!mode.startsWith("-") && input_file == null) {
                input_file = mode; // Assume this is the input file
            } else {
                System.err.println("Unknown option or extra input: " + mode);
                success = false;
                break;
            }
        }

        if (!success || input_file == null) {
            System.err.println("Usage:");
            System.err.println("  java soct.Main [-m <module_name>] [-o <output_file>] <input_file>");
            System.exit(1);
        }
        ...
        try {
            ...
            if (rocket_system != null) {
                if (dmi_bus != null && riscv_module_name.endsWith("e")) {
                    bscan_bus = new Bus();
                    String[] sigs = {
                            "capture",
                            "drck",
