...
trait BdPinPort extends ConnectOps {
   ...
   * @throws soct.vivado.VivadoDesignException if the builder is not locked yet (instance names would be unstable)
   */
...
object BdPinPort {
   ...
   * @throws soct.vivado.VivadoDesignException if an externalized port endpoint is paired with a non-interface pin
   */
   ...
   * @throws soct.vivado.VivadoDesignException if an externalized port is paired with a non-interface pin or has more than one connection
   */
