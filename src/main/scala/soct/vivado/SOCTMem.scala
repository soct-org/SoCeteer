...
import soct.vivado.fpga.{DDR4PortParams, PartRegistry}
 ...
 *   - capacities are derived from the part names via [[soct.vivado.fpga.PartRegistry]]
...
object SOCTMem {
   ...
   * @throws soct.vivado.VivadoDesignException if `maxSize` is negative or a port has no capacity
   */
   ...
   * @throws soct.vivado.VivadoDesignException if the requested parts do not match the
