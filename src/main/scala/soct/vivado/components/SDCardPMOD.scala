...
import soct.vivado.misc.{BasePMODPin, DTSInfo, DigilentPMODPin, WantsPMODPins}
...
case class SDCardPMOD(
   ...
   * design sources.
   *
   * @throws soct.vivado.VivadoDesignException if a bundled Verilog resource is missing
   */
  ...
  /**
   * @throws soct.vivado.VivadoDesignException if the DTS info does not carry exactly one register region
   */
