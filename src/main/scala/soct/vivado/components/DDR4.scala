...
import soct.vivado.fpga.{FPGADiffClockPort, FPGAResetPortSource, PartRegistry}
import soct.vivado.{SOCTBdBuilder, StringToTCLCommand, TCLCommands, VivadoDesignException}
import soct.system.vivado.DDR4Info
...
case class DDR4(info: DDR4Info)(implicit bd: SOCTBdBuilder, p: Parameters)
  ...
  /**
   * @throws soct.vivado.VivadoDesignException if custom mode has no memory part set,
  ...
  /**
   * @throws soct.vivado.VivadoDesignException if custom mode is active but the board
  ...
  /**
   * @throws soct.vivado.VivadoDesignException if the address aperture is not positive
   */
