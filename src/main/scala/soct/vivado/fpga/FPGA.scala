...
import soct.vivado.abstracts.{HasFriendlyName, IsXilinx}
...
abstract class FPGA extends IsXilinx with HasFriendlyName {
   ...
   * @throws soct.vivado.VivadoDesignException if the specified PMOD port or pin index is not defined for this FPGA board
   ...
   * @throws soct.vivado.VivadoDesignException if the board cannot provide `n` clock inputs
   */
