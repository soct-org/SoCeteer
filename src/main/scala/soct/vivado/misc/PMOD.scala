...
import soct.vivado.abstracts.{BdPinPort, EmitsConstraint}
...
trait BasePMODPin {
   ...
   * @throws soct.vivado.VivadoDesignException if this pin has no raw equivalent (power/ground pins, out of range)
   */
   ...
   * @return the FPGA pin
   * @throws soct.vivado.VivadoDesignException if the pin has no raw equivalent or the board does not define the PMOD port
   */
