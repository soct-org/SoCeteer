...
import soct.vivado.{TCLCommands, VivadoDesignException}
import soct.vivado.abstracts.BdIntfPin
 ...
 * [[soct.vivado.components.AXIIntc]] for why the PLIC cannot take them directly).
 * @param index
...
object AddressSets {
   ...
   * @throws soct.vivado.VivadoDesignException if `rangeBytes` is not positive
   */
...
object AxiSlaveBinder {
   ...
   * @throws soct.vivado.VivadoDesignException if `dts` has no compatibles or no register regions
   */
