...
class BDBuilder() {
  ...
  def generateInitScript(): String = {
    checkInit()
    implicit val p: Parameters = this.p.get
    val paths = p(HasSOCTPaths).asInstanceOf[BoardSOCTPaths]
    val config = p(HasSOCTConfig)
    val fpga = p(HasXilinxFPGA).get
    val projectName = s"soct-${config.configName}-${fpga.friendlyName.replaceAll("-", "_")}"
    val sources = "sources_1"
    ...
    s"""# Auto-generated Vivado TCL script
       ...
       |  create_project -part ${fpga.xilinxPart} $projectName ${paths.vivadoProjectDir}
       |  set_property board_part ${fpga.partName} [current_project]
       |}
       |
       |# Create "$sources" fileset (if not found)
       |if {[llength [get_filesets -quiet $sources]] == 0} {
       |  create_fileset -srcset $sources
       |}
       |
       |set source_fileset [get_filesets $sources]
       |add_files -fileset $$source_fileset ${paths.systemDir}
       |
       |""".stripMargin
  }
}
...
object SOCTVivado {
  ...
  def generateProject(tclFile: Path, vivado: Path): Unit = {
    val cmd = s"${vivado.toAbsolutePath} -mode batch -source ${tclFile.toAbsolutePath}"
