...
void enable_fpu() {
  uint64_t mstatus;
  asm volatile ("csrr %0, mstatus" : "=r" (mstatus)); // Read mstatus CSR

  mstatus |= 0x6000; // Set FS field to 11 (bits 13:12)
  asm volatile ("csrw mstatus, %0" :: "r" (mstatus)); // Write back to mstatus CSR
}
