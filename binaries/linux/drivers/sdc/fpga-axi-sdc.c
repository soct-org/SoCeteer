...
//
// From eugene-tarassov/vivado-risc-v (patches/fpga-axi-sdc.c) - the Linux driver for the
// same project's axi_sdc_controller, whose RTL this SoC instantiates (vendored under
// src/main/resources/sdc). Local changes, kept minimal so upstream syncs stay diffable:
...
static int sdc_get_cd(struct mmc_host * mmc) {
    ...
    /* With an active-low detect switch (DTS "cd-inverted" -> MMC_CAP2_CD_ACTIVE_HIGH),
     * the controller's debounced presence level reads inverted: a present card shows as
     * the "remove" state. Only this test carries polarity - the insert/remove interrupt
     * arming elsewhere is symmetric (watch for whichever transition, re-arm the other)
     * and needs no awareness. */
    if (mmc->caps2 & MMC_CAP2_CD_ACTIVE_HIGH)
        return (card_detect & SDC_CARD_REMOVE_INT_REQ) != 0;
