...
const SOCT_GLOSSARY = [
  ...
  },
  {
    ...
    entries: [
      ...
      { t: ["newlib"], d: "The embedded C standard library (SoCeteer links its 'nano' variant); small, but with quirks like a byte-wise memcpy and a 2 GiB allocation limit. Targets bare metal - it cannot make Linux syscalls, which is why Linux userspace programs use musl instead." },
      { t: ["musl"], d: "A lightweight, static-linking-friendly C standard library for Linux. The linux-sysroot build compiles it from source with the LLVM toolchain; Linux userspace programs link it statically." },
